...
#
# 末尾の「阿波踊り」「鳴潮連」は誤検出対策。徳島の阿波踊りには『鳴潮連』という
# 連(踊りのグループ)があり、ゲームとは無関係の記事が「鳴潮」の部分一致で拾われる。
# 手がかり語が固有名詞として別の意味を持つ例。
OFF_TOPIC_MARKERS = (
    "zenless", "zenlesszonegame", "ゼンレスゾーンゼロ", "原神", "genshin",
    "スターレイル", "starrail", "崩壊3rd", "崩壊スターレイル", "honkai",
    "アズールレーン", "fgo",
...
# コスプレ写真集・ファンアート・フィギュア情報などが該当する。
#
# ⚠️ 広い語を入れてはいけない。実データに当てて確認すること。
# 過去に誤爆した例:
#   「レイヤー」 → 「プレイヤー」に一致し、「プレイヤースキル次第で最強火力が
#                 出るキャラ3選」のような正当なネタが落ちていた
#   「写真集」   → ゲーム内スクショ集(「"自分の好み"を詰め込んだエイメス写真集」)は
#                 ボイスなし動画としてそのまま成立する正当なネタ
#   「イラスト」 → 公式の「Ver.3.5 ストーリーイラスト壁紙」のような告知が落ちる
#   「衣装」     → ゲーム内の新衣装は正当なネタ
# 実写・二次創作・物販を名指しする語だけに絞る。
NOISE_MARKERS = (
    "コスプレ", "こすぷれ", "cosplay", "グラビア", "水着グラビア",
    "漫画博覧会", "ワンダーフェスティバル", "ワンフェス", "コミケ", "コミックマーケット",
    "フィギュア",
    "描いてみた", "落書き", "らくがき", "一発描き", "だけで描いた", "絵師",
    "ファンアート", "fanart", "fan art", "イラストメイキング",
)
...
class IntelligenceEngine:
    ...
    def update_schedule_if_stale(self, stale_after_hours=20):
        ...
        prompt = (
            ...
            "2. わかる場合は end_date（バナー終了日・イベント終了日等）も入れる。\n"
            "   個別の終了日が書かれていなくても、そのバージョンの終了日が分かる場合は"
            "それを end_date に入れてよい（ガチャ・イベントはバージョン終了と同時に終わるため）。\n"
            "   どちらも読み取れない場合だけ null。カレンダー上で幅を持てず名前が読めなくなるので"
            "可能な限り埋めること。\n"
